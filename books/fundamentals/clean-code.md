...
- Avoid encodings.
  ...
  - Member prefixes - No need for m\_ anymore. Functions that they are in should be small anyway. IDEs will just highlight them for you.
...
- Method names
  ...
  - **_Accessors, mutators, and predicates should be named for their value and prefixed with get, set, and is according to the javabean standard._**
  - **_When constructors are overloaded, use static factory methods with names that describe the arguments._**
...
- Therefore, objects and data structures are opposites.
  - Example: A Geometry object-oriented class with simple shape data structures (Square, Rectangle, and Circle).
...
- Try to depend on abstractions, not concretions.
...
- Wouldn't manage all the details yourself -- teams of people who manage a specific part of the city.
  - Some are responsible for the big picture, but some focus on the details.
